...
{
  // std::cout << "b_12:" << b_12 << "\t" << "b_34:" << b_34 << "\t"
  //	    << "b_23:" << b_23 << "\t" << "b_41:" << b_41 << std::endl;
...
void SIMULATOR::Signal2(Double_t power, Double_t detuning, Double_t Decay, Double_t cos_solid_angle, Double_t solid_angle, Double_t state13, Double_t state24, bool flag){
  if(flag) std::cout << std::string(60, '~') << std::endl;
  Double_t Gamma = sqrt(4*pow(pi,2.)*pow(detuning, 2.)+4*pow(power, 2.)); // MuSEUM technical note (2.50)
  Double_t L = 2*pow(power, 2.)/(pow(Gamma, 2.) + pow(gamma, 2.));

  Double_t pola13_on = (pow(state13,2.)*(pow(TMath::Cos(0.5*Gamma*Decay),2.)+pow(detuning*TMath::Sin(0.5*Gamma*Decay)/Gamma,2.))+pow(state24*2*power*TMath::Sin(0.5*Gamma*Decay)/Gamma,2.)+4*state13*state24*power*detuning*pow(TMath::Sin(0.5*Gamma*Decay)/Gamma,2.))*TMath::Exp(-gamma*Decay);
  Double_t pola24_on = (pow(state13*2*power*TMath::Sin(0.5*Gamma*Decay)/Gamma,2.)+pow(state24,2.)*(pow(TMath::Cos(0.5*Gamma*Decay),2.)+pow(detuning*TMath::Sin(0.5*Gamma*Decay)/Gamma,2.))-4*state13*state24*power*detuning*pow(TMath::Sin(0.5*Gamma*Decay)/Gamma,2.))*TMath::Exp(-gamma*Decay);
  
  Double_t pola_on = pola13_on-pola24_on;
  Double_t pola_off = 2*(state13-state24)*TMath::Exp(-gamma*Decay);

  if(tree_TMmode=="TM210"){
    pola_on = -pola_on;
    pola_off = -pola_off;
  }
  ...
  Pon += 0.5*gamma*pow(y,2.)*(TMath::Exp(-gamma*Decay)*A[1]*solid_angle + A[0]*pola_on*cos_solid_angle)/TMath::Pi();
  Poff += 0.5*gamma*pow(y,2.)*(TMath::Exp(-gamma*Decay)*A[1]*solid_angle + A[0]*pola_off*cos_solid_angle)/TMath::Pi();
...
void SIMULATOR::CalculateSignal(bool FWHM_falg=false, Double_t bp=0., Double_t start=6.5, Double_t end=7.5){
  ...
  for(int w=0; w<scan_points; w++){
	...
	if((method=='c'||method=='C')&&FWHM_falg==false){
	  ...
	  signal += OldMuoniumSignal((*myField)[2], // power
                                     ...
                                     (*myMuonVec)[0]*1.0e-6+1, // decaytime + 0.001[ns]
                                     (*myAngleVec)[0], // cos_solid_angle                                                                                                                                 
                                     (*myAngleVec)[1], // solid_angle                                                                                                                                     
                                     false); // show the value
	  */
	  ...
	  */
		 ...
		 ( 0*1.0e+3<=(*myMuonVec)[0] && (*myMuonVec)[0]<=end*1.0e+3 )){ // cut positrons whose decaytime is not in the timewindow
    ...
    /*
    std::cout << "detuning:" << detuning << " [/kHz]" << "\n"
	      << "Non:" << Non << "\n"
	      << "Noff:" << Noff << std::endl;
    */
    /*
  ...
  */
  ...
  //else if(method=='o'||method=='O') c->SaveAs(("../figure/"+run_num+":"+tree_TMmode+":"+tree_Pressure+":"+"Old"+".png").c_str());
  //else if(method=='o'||method=='O') c->SaveAs(("../figure/oldm_tm110_2/"+tree_Pressure+":"+start_str+".png").c_str());
