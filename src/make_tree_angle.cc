...
{
  TString path = "../data/" + run_num + "_angle.root";
  TString MODE;
  TString ATM;
  TString KEL;
  
  if(gSystem->GetPathInfo(path, info)==0) std::cout << run_num + "_angle is already exist" << std::endl;
  else{    
    flag = true;
    ...
    file = new TFile(("../data/"+run_num+"_angle.root").c_str(),"RECREATE");
    
    if(DecayTree->Write()) std::cout  << run_num << "_angle.root is made." << std::endl;
    file->Close();
  }
}
