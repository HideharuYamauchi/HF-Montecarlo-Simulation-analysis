...
void muonstopping::Vis_stopping_distZ(){
  ...
  for(Int_t n=0;n<entries;n++){
    tree->GetEntry(n);
    if(((std::string(particle)=="mu+")&&(std::string(process)=="DecayWithSpin"))||(std::string(particle)=="e+")&&(std::string(volume)=="Cavity")) dtz->Fill(Z-1000.,Y);
  }
  dtz->Draw("Colz");
...
int muonstopping::GetNumber(Double_t* pos){
  ...
  */
  for(int i=0;i<entries;i++){
    tree->GetEntry(i);
    if((std::string(volume)=="Cavity")&&(std::string(particle)=="mu+")){
