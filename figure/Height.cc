...
//#include <fstream>
{
  ...
  std::string filename;  
  if(method=='c'||method=='C') filename = "FWHM_conv_hight.dat";
  else if (method=='o'||method=='O') filename = "FWHM_old_hight.dat";
  ...
  mg->GetYaxis()->SetTitle("Singal_{MAX}");
  
  if(method=='c'||method=='C') c3->BuildLegend(0.15,0.65,0.39,0.8);
  else if(method=='o'||method=='O') c3->BuildLegend(0.6,0.3,0.84,0.45);
  
  if(method=='c'||method=='C') c3->SaveAs("FWHM_conv_Hight.png");
  else if(method=='o'||method=='O') c3->SaveAs("FWHM_old_Hight.png");
}
